...
class Connection(object):
    ...
    :param local_infile: Boolean to enable the use of LOAD DATA LOCAL command.
        ...
        will be looked up in the dictionary and the retrieved PEP-343 object uses '__enter__' to
        return a file like object (has 'read' method) that will be used to retrieve bytes (like
...
class LoadLocalFile(object):
    ...
    def send_data(self):
        ...
        try:
            try:
                with conn._local_infile[self.filename] as open_file:
                    while True:
                        chunk = open_file.read(packet_size)
                        if not chunk:
                            break
                        conn.write_packet(chunk)
            # Boolean True[file] will return TypeError
            except (TypeError):
